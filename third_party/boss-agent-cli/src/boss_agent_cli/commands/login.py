...
def _classify_login_error(exc: Exception, ctx: click.Context) -> dict[str, object]:
	...
	if isinstance(exc, AccountRiskLocked):
		return {
			"code": "ACCOUNT_RISK",
			"message": raw_message,
			"recoverable": False,
			...
			"hints": {"next_actions": [
				"不要通过 CDP、patchright 或 Bridge 重试登录",
