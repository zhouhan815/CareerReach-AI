...
def handle_auth_errors(command_name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
	...
	def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
		...
		def wrapper(ctx: Any, *args: Any, **kwargs: Any) -> Any:
			from boss_agent_cli.api.browser_client import BrowserSessionRequired, RecruiterChatTabRequired
			...
			except BrowserSessionRequired as e:
				handle_error_output(
					ctx, command_name, code="BROWSER_SESSION_REQUIRED",
					message=str(e),
					recoverable=True,
					recovery_action="启动固定 Edge/Chrome CDP 窗口，登录 BOSS 官方页面后重试",
					hints={"next_actions": [
						r'& "D:\Codex project\job\.agents\skills\boss-job-agent\scripts\launch-edge-cdp.ps1"',
						"boss --cdp-url http://127.0.0.1:9222 login --cdp",
						"boss --cdp-url http://127.0.0.1:9222 status --live",
					]},
				)
			...
			except AccountRiskLocked as e:
				handle_error_output(
					ctx, command_name, code="ACCOUNT_RISK",
					message=str(e),
					recoverable=False,
					recovery_action="停止自动化访问；回到 BOSS 直聘官方页面手动处理账号状态，确认恢复后再清除本地风险锁",
					hints={"next_actions": [
						"不要通过 CDP、patchright 或 Bridge 重试该操作",
						"仅在官方页面确认账号恢复后，手动清除本地风险锁",
					]},
				)
